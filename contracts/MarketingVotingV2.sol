...
contract MarketingVotingV2 is PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function revokeVote() external nonReentrant {
        ...
        address lastAccount = _optionVoters[_optionVoters.length - 1];

        optionMapVoters[index][lastAccount] = indexInOV;
        optionVoters[index][indexInOV] = lastAccount;
    ...
    function fixVotes() external onlyRole(CONFIGURATOR_ROLE) {
        for (uint256 i = 0; i <= optionsCount; i++) {
            address[] memory _optionVoters = optionVoters[i];
            for (uint256 indexInOV = 0; indexInOV < _optionVoters.length; indexInOV++) {
                address account = _optionVoters[indexInOV];
                optionMapVoters[i][account] = indexInOV;
            }
        }
    }
