...
contract MarketingVotingV5 is PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    event SnapshottedResult(uint256 indexed index, uint256 votes, uint256 indexed optionIndex);
    ...
    function voteForOption(uint256 index, bytes memory signature, address account) external onlyRole(PROXY_VOTER_ROLE) {
        require(options[index].active, "MarketingVoting: option is not active");
    ...
    function revokeVote() external {
        ...
        for (uint256 i = 0; i < _optionVoters.length; i++) {
            if (_optionVoters[i] == account) {
                optionVoters[index][i] = _optionVoters[_optionVoters.length - 1];
    ...
    function createVoteSnapshot() external onlyRole(SNAPSHOTER_ROLE) {
        for (uint256 i = 0; i < options.length; i++) {
            ...
            if (options[i].active) {
                ...
                emit SnapshottedResult(snapshotsCount, votes, i);
            }
        }
        snapshotsCount++;
    }
