...
contract VEGoMiningTokenV5 is IVEGoMiningToken, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    IMintRewardV3 public MintReward;
//    uint256[49] __gap;
    ...
    function createLock(uint256 value, uint256 _unlockTime) external override nonReentrant whenNotPaused {
        ...
        MintReward.setLastClaim(_msgSender());
    }
...
//    function withdrawButIgnoreRewards() external override nonReentrant {
//
//        LockedBalance memory _locked = locked[_msgSender()];
...
//    }
    ...
    function withdraw() external override nonReentrant {
        ...
        uint256 lastClaim = MintReward.lastClaim(_msgSender());
        require(lastClaim != 0, 've: Withdraw rewards first');

        (, , uint256 nextRewardBlk) = MintReward.rewards(lastClaim + 1);
        if (nextRewardBlk != 0) {
            require(balanceOfAt(_msgSender(), nextRewardBlk) == 0, 've: Withdraw rewards first');
        }
    ...
    function updateForV5(address _mintReward) external onlyRole(UPGRADER_ROLE) {
        MintReward = IMintRewardV3(_mintReward);
    }
